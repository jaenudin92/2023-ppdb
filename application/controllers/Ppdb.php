...
class Ppdb extends CI_Controller
{
	...
	{
		$data["title"] = "PPDB";
		$data['reguler'] = $this->db->query("select * from tbl_jadwal_pendaftaran where dibuka <= cast(now() as date) and ditutup >= cast(now() as date) and jalur = 'Reguler'")->row_array();
		$data['prestasi'] = $this->db->query("select * from tbl_jadwal_pendaftaran where dibuka <= cast(now() as date) and ditutup >= cast(now() as date) and jalur = 'Prestasi'")->row_array();
	...
	public function prosesreguler()
	{
		...
		$cekkuota = $this->db->query("select * from tbl_jadwal_pendaftaran where nama_formulir = '$formulir_pendaftaran' ")->row_array();
		$cekpendaftar = $this->db->query("select * from tbl_calon_siswa where formulir_pendaftaran = '$formulir_pendaftaran' ")->num_rows();
		if ($cekpendaftar >= $cekkuota['kuota']) {
			$response = [
				"status" => FALSE,
				"error" => 'kuota'
			];
